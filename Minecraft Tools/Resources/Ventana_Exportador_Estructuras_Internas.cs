    ...
    public partial class Ventana_Exportador_Estructuras_Internas : Form
    {
        ...
        private void Ventana_Plantilla_Shown(object sender, EventArgs e)
        {
            try
            {
                ...
                /*string[] Matriz_Rutas = Directory.GetFiles(@"", "*.png", SearchOption.TopDirectoryOnly);
                foreach (string Ruta in Matriz_Rutas)
                {
                    File.Move(Ruta, Path.GetDirectoryName(Ruta) + "\\Item_" + Path.GetFileName(Ruta).ToLowerInvariant());
                }*/
                string[] Matriz_Rutas = Directory.GetFiles(@"C:\Program Files (x86)\Image-Line\FL Studio 9\Data\Projects\____PROYECTOS\EXTENDED MIXES 3.0\18w19b\data\minecraft\recipes", "*.png", SearchOption.TopDirectoryOnly);
                foreach (string Ruta in Matriz_Rutas)
                {
                        ...
                        foreach (string Ruta in Matriz_Rutas)
                        {
                            try
                            {
                                if (!string.IsNullOrEmpty(Ruta) && File.Exists(Ruta))
                                {
                                    ...
                                    {
                                        if (string.Compare(Extensión, ".jar", true) == 0) // A Minecraft version.
                                        {

                                        }
                                        else // Any NBT structure file.
                                        {
