...
{
    public partial class Ventana_Registro_Semillas_Útiles: Form
    {
        public Ventana_Registro_Semillas_Útiles()
        {
            InitializeComponent();
        }

        [StructLayout(LayoutKind.Sequential)]
        internal struct Semillas
        {
            internal string Texto;
            internal long Número;
            internal string Versión;
            internal string Descripción;

            internal Semillas(string Texto, long Número, string Versión, string Descripción)
            {
                this.Texto = Texto;
                this.Número = Número;
        ...
        internal readonly string Texto_Título = "Useful Seeds Registry for " + Program.Texto_Usuario + " by Jupisoft";
        ...
        private void Ventana_Registro_Semillas_Útiles_Load(object sender, EventArgs e)
        {
            try
            {
                this.Icon = Program.Icono_Jupisoft.Clone() as Icon;
                this.Text = Texto_Título + " - [Please send me or post your best seeds to add them here]";
                this.WindowState = FormWindowState.Maximized;
                Ocupado = true;
                Registro_Cargar_Opciones();
                Ocupado = false;
                if (Matriz_Semillas != null && Matriz_Semillas.Length > 0)
                {
                    foreach (Semillas Semilla in Matriz_Semillas)
                    {
                        ComboBox_Semilla.Items.Add(Semilla.Texto + ", " + Semilla.Número);
                    }
                }
                if (ComboBox_Semilla.Items.Count > 0) ComboBox_Semilla.SelectedIndex = 0;
            }
        ...
        private void Ventana_Registro_Semillas_Útiles_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
        ...
        private void Ventana_Registro_Semillas_Útiles_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
        ...
        private void Ventana_Registro_Semillas_Útiles_DragEnter(object sender, DragEventArgs e)
        {
            try
            {
        ...
        private void Ventana_Registro_Semillas_Útiles_DragDrop(object sender, DragEventArgs e)
        {
            try
            {
        ...
        private void Ventana_Registro_Semillas_Útiles_SizeChanged(object sender, EventArgs e)
        {
            try
            {
        ...
        private void Ventana_Registro_Semillas_Útiles_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void ComboBox_Semilla_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (ComboBox_Semilla.SelectedIndex > -1)
                {
                    TextBox_Descripción.Text = Matriz_Semillas[ComboBox_Semilla.SelectedIndex].Descripción;
                }
                else TextBox_Descripción.Text = null;
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }
        ...
        private void Botón_Copiar_Texto_Click(object sender, EventArgs e)
        {
            try
            {
                if (ComboBox_Semilla.SelectedIndex > -1)
                {
                    Clipboard.SetText(Matriz_Semillas[ComboBox_Semilla.SelectedIndex].Texto);
                    SystemSounds.Asterisk.Play();
                }
        ...
        private void Botón_Copiar_Número_Click(object sender, EventArgs e)
        {
            try
            {
                if (ComboBox_Semilla.SelectedIndex > -1)
                {
                    Clipboard.SetText(Matriz_Semillas[ComboBox_Semilla.SelectedIndex].Número.ToString());
                    SystemSounds.Asterisk.Play();
                }
                else SystemSounds.Beep.Play();
            }
